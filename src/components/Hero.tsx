...
export default function Hero() {
  ...
  const inp =
    "w-full px-4 py-3 rounded-xl border border-gray-200 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent/40 transition-all";

  return (
    <section
      id="hero"
      className="relative min-h-[100vh] flex items-center bg-gradient-to-b from-primary via-primary to-primary-dark overflow-hidden"
    >
      <div
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          <div>
            ...
            <div className="mt-8 flex items-center gap-3">
              <a
                href="tel:932844460"
                className="inline-flex items-center gap-3 text-white/60 hover:text-white font-medium text-base py-3 transition-colors"
                aria-label={
                  locale === "pt" ? "Ligar 932 844 460" : "Call 932 844 460"
                }
              >
              ...
              <a
                href="https://wa.me/351932844460"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 text-[#25D366] hover:text-[#20BD5A] font-medium text-sm py-3 transition-colors"
              >
                <svg
                  className="w-5 h-5"
                  viewBox="0 0 24 24"
            ...
            <div className="mt-10 flex items-center gap-6">
              <div
                ...
                aria-label={
                  locale === "pt"
                    ? "Classificação 4.9 de 5 estrelas"
              ...
              <span className="text-white/60 text-sm">
                500+{" "}
                {locale === "pt" ? "mudanças realizadas" : "moves completed"}
              </span>
            </div>
          </div>
            ...
            {status === "success" ? (
              <div className="py-12 text-center">
                ...
                <h3 className="text-xl font-bold text-primary">
                  {locale === "pt" ? "Pedido enviado!" : "Request sent!"}
                </h3>
              ...
              <>
                <h3 className="text-xl font-bold text-primary mt-2 mb-6">
                  {locale === "pt"
                ...
                <form onSubmit={handleSubmit} className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        name="phone"
                        required
                        placeholder={locale === "pt" ? "Telefone *" : "Phone *"}
                        className={inp}
                      />
                    </div>
                    <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label htmlFor="hero-from" className="sr-only">
                        {locale === "pt" ? "Cidade de origem" : "Origin city"}
                      </label>
                      <input
                        id="hero-from"
                        type="text"
                  ...
                  <div>
                    <label htmlFor="hero-date" className="sr-only">
                      {locale === "pt" ? "Data da mudança" : "Moving date"}
                    </label>
                    <input
                      id="hero-date"
                      type="date"
                      name="date"
                      className={`${inp} text-gray-400`}
                    />
                  </div>
                  <label className="flex items-start gap-2 text-xs text-gray-400 cursor-pointer">
                    ...
                    <span>
                      {locale === "pt"
                        ? "Aceito a recolha dos meus dados para resposta ao pedido de orçamento."
                        : "I consent to data collection to respond to my quote request."}
                    </span>
                  </label>
                  <button
                    type="submit"
                    disabled={status === "sending"}
                    className="block w-full bg-accent hover:bg-accent-hover disabled:opacity-50 text-white text-center font-bold py-3.5 rounded-xl transition-colors shadow-md shadow-accent/20"
                  >
                    {status === "sending"
                      ? locale === "pt"
                        ? "A enviar..."
                ...
                {status === "error" && (
                  <p className="text-red text-xs text-center mt-3" role="alert">
                    {locale === "pt"
                      ? "Erro ao enviar. Tente ligar 932 844 460."
                      : "Error sending. Try calling 932 844 460."}
                  </p>
                )}
                <p className="text-xs text-gray-400 text-center mt-4">
                  {locale === "pt"
                    ? "Resposta em menos de 24h · Sem compromisso"
                    : "Response within 24h · No obligation"}
                </p>
              </>
            )}
          </div>
      ...
      <div className="absolute bottom-0 left-0 right-0" aria-hidden="true">
        <svg
          ...
          className="w-full"
        >
          <path d="M0 60V30C360 0 720 0 1080 30L1440 60H0Z" fill="white" />
        </svg>
      </div>
    </section>
  );
