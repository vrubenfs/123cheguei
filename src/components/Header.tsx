...
export default function Header() {
  ...
  return (
    <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-500 ${scrolled ? "bg-white/95 backdrop-blur-lg shadow-md shadow-primary/5" : "bg-transparent"}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16 sm:h-[72px]">
        <a href="#" aria-label="123cheguei"><Logo variant={scrolled ? "default" : "white"} /></a>

        <nav className="hidden md:flex items-center gap-1">
          {links.map(l => (
            <a key={l.href} href={l.href} className={`text-sm font-medium px-3 py-2 rounded-lg transition-all ${scrolled ? "text-foreground/60 hover:text-primary hover:bg-primary/[0.04]" : "text-white/60 hover:text-white hover:bg-white/[0.06]"}`}>{l.label}</a>
          ))}
        </nav>

        <div className="flex items-center gap-2">
          <button onClick={toggleLocale} className={`text-xs font-bold border rounded-full w-9 h-9 flex items-center justify-center transition-all ${scrolled ? "border-primary/15 text-primary/70 hover:bg-primary/[0.04]" : "border-white/20 text-white/70 hover:bg-white/[0.06]"}`} aria-label={locale === "pt" ? "Switch to English" : "Mudar para Português"}>
            {locale === "pt" ? "EN" : "PT"}
          </button>
          ...
          <button className={`md:hidden w-10 h-10 flex items-center justify-center rounded-lg ${scrolled ? "text-primary" : "text-white"}`} onClick={() => setMenuOpen(!menuOpen)} aria-label="Menu">
